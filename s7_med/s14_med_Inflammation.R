...
Pro <- fread(paste0(data_dir, '/Inflammation/processed/Inflammation.csv'))
colnames(Pro)
Pro[, names(Pro) := lapply(.SD, function(x) {
  x[is.infinite(x)] <- NA
  return(x)
})]
...
x_m <- fread('results/s4_med/Score_Inflammation.csv')
m_y <- fread('results/s4_med/Infla_death.csv')
...
for (pro in pro_lst) {
  ...
  if (count %% 1 == 0) {
    chunk_path <- paste0("results/s4_med/chunks/Inflammation_med_chunk_", pro, ".csv")
...
all_chunks <- list.files(
  ...
  pattern = "^Inflammation_med_chunk_.*\\.csv$",
  full.names = TRUE
)
...
fwrite(all_results, "results/s4_med/Inflammation_med.csv", row.names = FALSE)
cat("All chunks combined and saved to Inflammation_med")
