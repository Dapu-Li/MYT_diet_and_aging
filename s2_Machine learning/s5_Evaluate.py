...
def get_avg_output(mydf, gt_col, pred_col, cutoff, nb_iters):
    ...
    output_lst = []
    for i in range(7):
        output_lst.append('{:.3f}'.format(result_df.iloc[i]['Median']) + ' [' +
                          '{:.3f}'.format(result_df.iloc[i]['LBD']) + ' - ' +
                          '{:.3f}'.format(result_df.iloc[i]['UBD']) + ']')
...
tgt_dir_lst = sort_nicely(glob.glob(os.path.join(root_path, 'Results/predict/ba/PRED_*.csv')))

save_dir = os.path.join(root_path, 'Results/predict/ba/Evaluation')
os.makedirs(save_dir, exist_ok=True)
tgt_dir = tgt_dir_lst[0]



def process_file(tgt_dir, nb_iters):
    tgt = os.path.basename(tgt_dir)[5:-4]
    ...
    try:
        ...
        res_pro = get_avg_output(tgt_pred_df, 'target_y', 'y_pred_pro', ct_pro, nb_iters=nb_iters)
        res_cov = get_avg_output(tgt_pred_df, 'target_y', 'y_pred_cov', ct_cov, nb_iters=nb_iters)
        res_pro_cov = get_avg_output(tgt_pred_df, 'target_y', 'y_pred_pro_cov', ct_pro_cov, nb_iters=nb_iters)
        res_df = pd.concat([res_pro, res_cov, res_pro_cov], axis=1)
        res_df = res_df.T
        res_df.index = ['Protein', 'Demographic', 'Protein+Demographic']
        res_df.to_csv(os.path.join(save_dir, f'{tgt}.csv'))
    except Exception as e:
        print(f"Error processing {tgt}: {e}")
        with open(os.path.join(save_dir, f'error_{tgt}.txt'), 'a') as f:
            f.write(f"{tgt}: {e}\n")
        return
...
with parallel_backend("loky", n_jobs=nb_cpus):
    Parallel(n_jobs=nb_cpus)(
        delayed(process_file)(tgt_dir, nb_iters) for tgt_dir in tqdm(tgt_dir_lst)
    )
